...
const Testimonials: React.FC<TestimonialsProps> = ({ onStartQuestionnaire }) => {
  ...
  // Testimoniales predeterminados como respaldo
  ...
  // Testimoniales hardcodeados para el modal
  const testimonialSections = [
    {
      ...
      testimonials: [
        ...
        },
        {
          id: 'modal3',
          name: '📱 David, 45 años',
          comment: 'Hola, muy buenas Jose, gracias por tu interés. Estoy comiendo mejor que nunca y sin rayarme 🤯 Pensé que definir era comer arroz y pollo, pero con este plan estoy disfrutiendo cada comida. Ya me noto más delgado, mantengo la fuerza en el gym y no me aburro. ¡Esto es otro nivel! 👌🔥',
          rating: 5
        }
      ]
    },
  ...
  const fetchApprovedTestimonials = async () => {
    try {
      ...
      if (error) {
        ...
        // Usar testimoniales predeterminados si hay error
        setApprovedTestimonials(defaultTestimonials);
      } else {
        // Si hay testimoniales de la base de datos, usarlos, sino usar los predeterminados
  ...
  return (
    <section id="testimonios" className="py-16 bg-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8 relative z-10 max-w-7xl">
        ...
        {/* Show testimonials from database or default ones */}
        {loading ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mb-12">
            {[...Array(4)].map((_, index) => (
              <div key={index} className="bg-gray-50/90 backdrop-blur-sm rounded-xl md:rounded-2xl p-5 md:p-6 shadow-lg border border-nutrition-green-light animate-pulse">
