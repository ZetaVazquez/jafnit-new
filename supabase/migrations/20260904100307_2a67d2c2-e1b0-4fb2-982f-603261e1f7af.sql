...
BEGIN
  ...
  IF p_plan_type IS NOT NULL THEN
    ...
    IF v_subscription_id IS NULL THEN
      INSERT INTO public.subscriptions (user_id, plan_type, status, start_date, end_date, payment_method, amount)
      VALUES (p_user_id, p_plan_type, 'active', v_start, v_end, 'manual', v_amount)
      RETURNING * INTO updated_row;
    ELSE
    ...
    END IF;
  ELSE
    IF p_subscription_id IS NULL OR p_new_end IS NULL THEN
      RAISE EXCEPTION 'subscription and date are required';
    END IF;
